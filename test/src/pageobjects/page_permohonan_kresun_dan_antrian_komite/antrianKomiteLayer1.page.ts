...
/**
 * MIGRATION NOTE: semua `browser.pause(...)` dihapus. `waitForClickable`/`waitForDisplayed`
 * manual dihapus (auto-wait Playwright). `browser.execute((el) => el.click(), await locator)`
 * (JS-click untuk menghindari element-intercept) -> `locator.evaluate(el => el.click())`.
 */
export default class AntrianKomiteLayer1 extends BasePage {
  constructor(page: Page) {
    super(page);
  }

  get halAntrianKomiteLayer1(): Locator {
    return this.page.locator('h2').filter({ hasText: /^Verifikasi Persetujuan Komite Layer 1$/ });
  }

  get prosesAntrianKomiteLayer1(): Locator {
    return this.page.locator('span').filter({ hasText: /^Proses$/ }).first();
  }

  get resumeVerifikasiLayer1(): Locator {
    return this.page.locator('h4').filter({ hasText: /^Resume Verifikasi Persetujuan Kredit Layer 1$/ });
  }

  get catatanVerifikasiDokumen(): Locator {
    return this.page.locator('#layer3__content__body__dokumen__keteragan');
  }
  ...
  get catatanVerifikasiInterview(): Locator {
    return this.page.locator('#layer3__content__body__interview__keterangan');
  }

  get catatanVerifikasiData(): Locator {
    return this.page.locator('#layer3__content__body__data__keterangan');
  }
  ...
  get approveLayer1(): Locator {
    return this.page.locator('#btn_submit_approve');
  }

  get rejectLayer1(): Locator {
    return this.page.locator('#btn_submit_reject');
  }

  get successApprovalantrianKomite1(): Locator {
  ...
  async prosesNopenAntrianKomiteLayer1(): Promise<void> {
    await this.prosesAntrianKomiteLayer1.click();
  }

  /**
   * MIGRATION NOTE: loop klik semua icon expand — `browser.pause(1000)` per iterasi
   * dihapus. Tidak ada dependency antar-icon (masing-masing accordion independen),
   * jadi klik berurutan tanpa delay statis aman; kalau salah satu icon belum
   * actionable, `.click()` Playwright akan retry otomatis sampai timeout default.
   */
  async dropdownDetailAntrianKomiteLayer1(): Promise<void> {
  ...
  async catatanVerifikasiDokumenLayer1(catatan: string): Promise<void> {
  ...
  async simpanverifikasiDokumenLayer1(): Promise<void> {
  ...
  /**
   * MIGRATION NOTE: `browser.execute((el) => el.click(), await this.interview_data_sesuai)`
   * -> `locator.evaluate(el => el.click())`. Escape hatch yang sama dipertahankan
   * (radio custom-styled), tapi lihat catatan di uiHelpers.ts — Playwright native click
   * kemungkinan sudah cukup; ini dipertahankan sesuai kode asli untuk parity.
   */
  ...
  async submitHasilVerifikasiLayer1(aksi: string): Promise<void> {
    switch (aksi.toLowerCase()) {
      case 'approve':
        await this.approveLayer1.click();
        await this.popupYaButton.click({ timeout: 10_000 });
        break;
      case 'reject':
        await this.rejectLayer1.click();
        break;
      default:
  ...
  async closePopupLayer1(): Promise<void> {
